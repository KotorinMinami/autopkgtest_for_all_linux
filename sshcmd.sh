...
function sshcmd() {
    ...
    test "$cmd"x = ""x && echo "Lack of execute command." && exit 1
    cmd=${cmd//\$/\\\$}

    test "$remoteip"x = ""x && echo "Missing ip."
    test "$(echo ${remoteip} | awk -F"." '{if ($1!=0 && $NF!=0) split ($0,IPNUM,".")} END { for (k in IPNUM) if (IPNUM[k]==0) print IPNUM[k]; else if (IPNUM[k]!=0 && IPNUM[k]!~/[a-z|A-Z]/ && length(IPNUM[k])<=3 && IPNUM[k]<=255 && IPNUM[k]!~/^0/) print IPNUM[k]}' | wc -l)" -ne 4 && echo "the remote ip is Incorrect." && exit 1
    if ping -c 1 ${remoteip} | grep "100% packet loss"; then
        echo "connection to $remoteip failed."
        exit 101
    fi

    test "$remoteuser"x = "root"x && echo "the remote user uses the default configuration."

    test "$remotepasswd"x = "openEuler12#$"x && echo "the remote password uses the default configuration."

    test "$timeout"x = "15"x && echo "the timeout uses the default configuration."

    test "$connport"x = "22"x && echo "the connect port using the default configuration"
