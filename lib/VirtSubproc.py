...
def wait_booted(
    ...
    long_command_timeout: int = 60,
) -> None:
    '''
    Wait until a container or VM with an init system has sufficiently
    booted to interact with it.

    timeout: how long we will continue to poll the container for status

    short_command_timeout: how long we will wait for a relatively quick command
    to finish

    long_command_timeout: how long we will wait for a slower command to finish
    '''
    ...
    while timeout > 0:
        timeout -= 1
        time.sleep(1)
        (rc, out, _) = execute_timeout(
            None, 10, exec_command + ['runlevel'],
            stdout=subprocess.PIPE,
        )
        if rc != 0:
            adtlog.debug('wait_booted: polling status failed, retrying...')
            continue
        out = out.strip()
        if out.split()[-1].isdigit():
            adtlog.debug('waiting for network')
            check_exec(
                exec_command + ['sh', '-ec', await_boot],
    ...
    try:
        ...
        for sdn in [1, 0]:
            ...
            if not (status == 0 or (sdn == 0 and status == -13)):
                timeout_stop()
                adtlog.info("%s %s failed, status %d" %
